...
| `OMBRE_BIND_HOST` | 否 | `0.0.0.0` | HTTP/SSE 监听地址；裸机仅经本地反代访问时建议设为 `127.0.0.1` |
...
| `OMBRE_DEHYDRATION_MODEL` | 否 | `deepseek-v4-flash` | 脱水/打标/合并/拆分用的 LLM 模型名（覆盖 `dehydration.model`）。`deepseek-chat`/`deepseek-reasoner` 已于 2026-07-24 起下线，统一指向 `deepseek-v4-flash`（非思考模式） |
...
| `OMBRE_EMBEDDING_API_KEY` | 否 | — | 向量嵌入专用 API Key（覆盖 `embedding.api_key`；留空则复用 `OMBRE_API_KEY`）。**若脱水模型（如 DeepSeek）和向量化模型（如 Gemini）不是同一家，必须单独设置这个，否则向量化会一直用错的 key 调用错的服务商，静默失败** |
